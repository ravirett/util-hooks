...
export function useSessionStorage<T>(key: string, initialValue: T | (() => T), saveInitial: boolean = false): [T, (value: T | ((val: T) => T)) => void, () => void] {
  ...
  const [storedValue, setStoredValue] = useState<T>(() => {
    ...
    if (!window || !window.sessionStorage) {
      console.error('Use of Session Storage is not supported in this environment');
    ...
    try {
      ...
      // sessionStore can't accept functions, so execute if it is
      ...
      if ((!item || item === 'undefined') || (!!item && !isNullOrUndefined(unwrappedInitialValue) && saveInitial)) {
        window.sessionStorage.setItem(key, JSON.stringify(unwrappedInitialValue));
      }
  ...
  useEffect(() => {
    const handleEvent = (event: StorageEvent) => {
      ...
      if (event.key !== key || event.storageArea !== window.sessionStorage) return;
